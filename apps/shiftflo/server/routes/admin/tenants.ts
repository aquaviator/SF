...
export function setupAdminTenantRoutes(app: Express) {
  ...
  app.get(
    "/api/admin/tenants",
    (req, res, next) => {
      console.log("🚀 ADMIN_TENANTS_ROUTE_HIT_BEFORE_AUTH", {
        timestamp: new Date(),
      });
      next();
    },
    ...
    async (req, res) => {
      try {
        console.log("🚀 ADMIN_TENANTS_ROUTE_HIT_AFTER_AUTH", {
          timestamp: new Date(),
        });
        ...
        if (status && status !== "all") {
          conditions.push(
            `COALESCE((SELECT status FROM subscriptions WHERE tenant_id = t.subdomain), 'inactive') = '${status}'`,
          );
        }

        if (search) {
          conditions.push(
            `(t.name ILIKE '%${search}%' OR t.subdomain ILIKE '%${search}%')`,
          );
        }
      ...
      `;
        ...
        console.log("🔍 EXECUTING_TENANTS_QUERY", {
          query: tenantsQuery,
          timestamp: new Date(),
        });
        ...
        console.log("✅ TENANTS_FETCHED", {
          count: tenantsData.length,
          timestamp: new Date(),
        });
        console.log("🔍 SAMPLE_TENANT_DATA", {
          sample: tenantsData[0],
          timestamp: new Date(),
        });
        console.log("🔍 TENANT_KEYS", {
          keys: Object.keys(tenantsData[0] || {}),
          timestamp: new Date(),
        });

        res.json(tenantsData);
      } catch (error) {
        console.error("❌ ADMIN_TENANTS_ERROR", {
          error: error.message,
          timestamp: new Date(),
        });
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        console.log("🔍 FETCHING_TENANT_DETAILS", {
          tenantId,
          adminId: req.admin?.id,
          timestamp: new Date(),
        });
        ...
        console.log("✅ TENANT_DETAILS_FETCHED", {
          tenantId,
          revenue: metrics.revenue,
          timestamp: new Date(),
        });
        ...
        console.error("❌ TENANT_DETAILS_ERROR", {
          error: error.message,
          timestamp: new Date(),
        });
  ...
  app.get(
    "/api/admin/tenants/cleanup-candidates",
    adminAuth,
    requireRole(["super_admin", "support"]),
    async (req, res) => {
      try {
        console.log("🏢 FETCHING_CLEANUP_CANDIDATES", {
          adminId: req.admin?.id,
          timestamp: new Date(),
        });
        ...
        console.error("❌ CLEANUP_CANDIDATES_ERROR", {
          error: error.message,
          timestamp: new Date(),
        });
  ...
  app.post(
    "/api/admin/tenants/cleanup-preview",
    adminAuth,
    ...
    async (req, res) => {
      try {
        ...
        if (!Array.isArray(tenantIds) || tenantIds.length === 0) {
          return res
            .status(400)
            .json({ message: "Invalid tenant IDs provided" });
        }
        ...
        const previews = await Promise.all(
          tenantIds.map(async (tenantId: number) => {
            ...
            const tableQueries = [
              ...
              },
              {
                ...
                query: db
                  .select({ count: sql<number>`count(*)` })
                  .from(shifts)
                  .where(eq(shifts.tenantId, tenant.subdomain)),
              },
              {
                name: "time_entries",
                query: db
                  .select({ count: sql<number>`count(*)` })
                  .from(timeEntries)
                  .where(eq(timeEntries.tenantId, tenant.subdomain)),
              },
              {
                ...
                query: db
                  ...
                  .where(eq(holidayRequests.tenantId, tenant.subdomain)),
              },
              {
                name: "assignments",
                query: db
                  .select({ count: sql<number>`count(*)` })
                  .from(assignments)
                  .where(eq(assignments.tenantId, tenant.subdomain)),
              },
              {
                name: "swap_requests",
                query: db
                  .select({ count: sql<number>`count(*)` })
                  .from(swapRequests)
                  .where(eq(swapRequests.tenantId, tenant.subdomain)),
              },
              {
                ...
                query: db
                  ...
                  .where(eq(businessProfiles.tenantId, tenant.subdomain)),
              },
              {
                name: "staff_strikes",
                query: db
                  .select({ count: sql<number>`count(*)` })
                  .from(staffStrikes)
                  .where(eq(staffStrikes.tenantId, tenant.subdomain)),
              },
              {
                name: "usage_metrics",
                query: db
                  .select({ count: sql<number>`count(*)` })
                  .from(usageMetrics)
                  .where(eq(usageMetrics.tenantId, tenant.subdomain)),
              },
              {
                ...
                query: db
                  .select({ count: sql<number>`count(*)` })
                  .from(seatAllocation)
                  .where(eq(seatAllocation.tenantId, tenant.subdomain)),
              },
              {
                name: "subscriptions",
                query: db
                  .select({ count: sql<number>`count(*)` })
                  .from(subscriptions)
                  .where(eq(subscriptions.tenantId, tenant.subdomain)),
              },
            ];
            ...
            const totalRecords = affectedTables.reduce(
            ...
            return {
              ...
              affectedTables: affectedTables.filter(
        ...
        console.error("❌ CLEANUP_PREVIEW_ERROR", {
          error: error.message,
          timestamp: new Date(),
        });
    ...
    async (req, res) => {
      try {
        ...
        for (const tenantId of tenantIds) {
          ...
          if (!tenant) {
            console.warn(`⚠️ TENANT_NOT_FOUND`, {
              tenantId,
              timestamp: new Date(),
            });
            continue;
          ...
          await db.insert(activityLogs).values({
            ...
            details: `Cleaning up tenant: ${tenant.name} (${tenant.subdomain})`,
          });
        ...
        console.error("❌ TENANT_CLEANUP_ERROR", {
          error: error.message,
          timestamp: new Date(),
        });
  ...
  app.post(
    "/api/admin/tenants/cleanup-report",
    adminAuth,
    requireRole(["super_admin", "support"]),
    async (req, res) => {
      try {
        ...
        res.setHeader(
          "Content-Disposition",
          `attachment; filename="tenant-cleanup-report-${new Date().toISOString().split("T")[0]}.csv"`,
        );
        res.send(csvContent);
      } catch (error) {
        console.error("❌ CLEANUP_REPORT_ERROR", {
          error: error.message,
          timestamp: new Date(),
        });
  ...
  app.put(
    ...
    async (req, res) => {
      try {
          ...
          await db
            ...
            .onConflictDoUpdate({
              ...
              set: {
                ...
                monthlyTotal: seatsIncluded
                  ? seatsIncluded * 300
                  : sql`${subscriptions.monthlyTotal}`,
              },
            });
        }
        ...
        console.error("❌ TENANT_UPDATE_ERROR", {
          error: error.message,
          timestamp: new Date(),
        });
  ...
  app.delete(
    ...
    async (req, res) => {
      try {
        ...
        console.log("✅ SINGLE_TENANT_DELETED", {
          tenantId,
          subdomain: tenant.subdomain,
          timestamp: new Date(),
        });
        ...
        console.error("❌ SINGLE_TENANT_DELETE_ERROR", {
          error: error.message,
          timestamp: new Date(),
        });
  ...
  app.get(
    ...
    async (req, res) => {
      try {
        ...
        console.log("👥 FETCHING_TENANT_USERS", {
          tenantId,
          adminId: req.admin?.id,
          timestamp: new Date(),
        });
        ...
        console.log("✅ TENANT_USERS_FETCHED", {
          tenantId,
    ...
    async (req, res) => {
      try {
        ...
        const [updatedUser] = await db
          ...
          .where(
            and(eq(users.id, userId), eq(users.tenantId, tenant.subdomain)),
          )
          .returning();
